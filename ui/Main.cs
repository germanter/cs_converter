...
namespace convix
{
    ...
    {
        // =========================================================================
        // CAT MOOD ENGINE CONFIGURATION PARAMETERS
        // =========================================================================
        private readonly int CatBlipSeconds = 15;        // Default: 15 (Minimum allowed: 1)
        private readonly int CatRestSeconds = 2;         // Default: 2  (Can be set to 0 to disable)
        private readonly int CatFuriousSeconds = 3;      // Default: 3  (Can be set to 0 to disable)
        private readonly int CatMaxWindowSeconds = 20;   // Default: 20 maximum total window sum
        private readonly double CatBlinkRateSeconds = 3.0; // Seconds between auto-blinks in 'blip' state
        // =========================================================================
        ...
        private List<MoodConfig> activeMoods = new();
        ...
        // Standard in-memory sub-system views cache to isolate category configurations cleanly
        private Dictionary<string, ICtgToolUI> _ctgViewsCache = new();
        ...
        private ICtgToolUI? GetOrCreateCtgView(string category)
        {
            if (!_ctgViewsCache.TryGetValue(category, out var view))
            {
                if (category == "Image2PDF")
                {
                    view = new Image2PdfUI(this, bgBrush, textBrush, globalFont);
                }
                ...
                else if (category == "PDFMerger")
                {
                    view = new PdfMergerUI(this, bgBrush, textBrush, globalFont);
                }
                else if (category == "Office2PDF")
                {
                    view = new Office2PdfUI(this, bgBrush, textBrush, globalFont);
                }
                
                if (view != null)
                {
                    _ctgViewsCache[category] = view;
                }
            }
            return view;
        }
        ...
        private void BuildUI()
        {
            ...
            foreach (var ctg in categories)
            {
                ...
                btn.Click += (s, e) => { 
                    ...
                    // Toggle settings box visibility and mount the selected category view
                    if (_ctgBoxHelper != null)
                    {
                        var view = GetOrCreateCtgView(activeCTG);
                        if (view != null)
                        {
                            _ctgBoxHelper.MountUI(view);
                            _ctgBoxHelper.IsVisible = true;
                        }
                        else
                        {
                            _ctgBoxHelper.IsVisible = false;
                        }
                    }
                };
            ...
            var defaultView = GetOrCreateCtgView("Image2PDF");
            if (defaultView != null)
            {
                _ctgBoxHelper.MountUI(defaultView);
            }
            
            _ctgBoxHelper.SetProgress(null); // Initialize clean
            _ctgBoxHelper.IsVisible = (activeCTG == "Image2PDF" || activeCTG == "ImageConverter" || activeCTG == "PDF2Image" || activeCTG == "PDFMerger" || activeCTG == "Office2PDF"); // Setup initial visibility configuration
        ...
        private Viewbox CreateAnimatedCat()
        {
            ...
            // Fallback Engine Configuration Guardrails
            int blipDur = CatBlipSeconds;
            int restDur = CatRestSeconds;
            int furiousDur = CatFuriousSeconds;
            int sum = blipDur + restDur + furiousDur;

            if (blipDur <= 0 || restDur < 0 || furiousDur < 0 || sum > CatMaxWindowSeconds || sum <= 0)
            {
                // Revert to factory specifications upon any invalid configurations
                blipDur = 15;
                restDur = 2;
                furiousDur = 3;
            }

            // Build active mood configuration tree
            activeMoods.Clear();
            if (blipDur > 0) activeMoods.Add(new MoodConfig { Name = "blip", Seconds = blipDur });
            if (restDur > 0) activeMoods.Add(new MoodConfig { Name = "rest", Seconds = restDur });
            if (furiousDur > 0) activeMoods.Add(new MoodConfig { Name = "furious", Seconds = furiousDur });

            blinkTimer = new DispatcherTimer { Interval = TimeSpan.FromSeconds(CatBlinkRateSeconds) };
            ...
            // Intercept mouse clicks on the cat visual frame without bubbling up to file picker triggers
            catViewbox.PointerPressed += (s, e) =>
            {
                e.Handled = true; 
                OnCatClicked();
            };
        ...
        private void StartMoodCycle()
        {
            if (activeMoods.Count == 0) return;
            currentMoodIndex = 0;
            ApplyMoodState(activeMoods[currentMoodIndex]);
        }

        private void ApplyMoodState(MoodConfig mood)
        {
            SetEyeState(mood.Name);
            if (moodTimer != null)
            {
                moodTimer.Stop();
                moodTimer.Interval = TimeSpan.FromSeconds(mood.Seconds);
                moodTimer.Start();
            }
        }
        ...
        {
            if (activeMoods.Count == 0) return;
            currentMoodIndex = (currentMoodIndex + 1) % activeMoods.Count;
            ApplyMoodState(activeMoods[currentMoodIndex]);
        }

        private void OnCatClicked()
        {
            // Reset mood rotation parameters to maintain state cycle alignment
            int blipIndex = activeMoods.FindIndex(m => m.Name == "blip");
            if (blipIndex >= 0)
            {
                currentMoodIndex = blipIndex;
                ApplyMoodState(activeMoods[currentMoodIndex]);
            }
            else
            {
            ...
            // Immediately animate single blink
            _ = TriggerSingleBlink();
        }
        ...
        {
            await TriggerSingleBlink();
        }

        private async Task TriggerSingleBlink()
        {
            try
            {
