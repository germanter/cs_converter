...
// /// <summary>
// /// High-performance serial image-to-PDF engine using a Two-Phase Disk-Buffered Pipeline.
// /// By processing images one-by-one to temporary files, this architecture completely eliminates 
// /// RAM hoarding and CPU thread deadlocks, capable of handling thousands of massive images seamlessly.
// /// </summary>
// public static class ImageToPdfEngine
// {
//     static ImageToPdfEngine()
//     {
//         // Disable OpenCL to prevent hardware-acceleration driver hangs on servers.
//         // We leave Thread limits to default so ImageMagick can natively multi-thread 
//         // the processing of a single image using all available CPU cores.
//         try { OpenCL.IsEnabled = false; } catch { /* Ignore if unavailable */ }
//     }
...
//     /// <summary>
//     /// Tracks the finalized files sitting on the disk ready for PDF binding.
//     /// </summary>
//     private record ProcessedFrameData(
//         string FilePath,
//         double PixelWidth,
...
//         // Create a totally isolated Temporary Directory on the OS hard drive
//         string tempDirectory = Path.Combine(Path.GetTempPath(), $"ImageToPdfEngine_{Guid.NewGuid():N}");
//         Directory.CreateDirectory(tempDirectory);

//         try
//         {
//             var processedFrames = new List<ProcessedFrameData>();
//             int tempFileCounter = 0;

//             // ====================================================================================
//             // PHASE 1: THE PRE-CONVERTER (One at a time)
//             // ====================================================================================
//             // This loop opens one image, makes it perfect, writes it to the temp folder, 
//             // and IMMEDIATELY disposes it from RAM before touching the next image.
...
//                 using var rawCollection = new MagickImageCollection(input.FilePath);
//                 if (rawCollection.Count == 0) continue;

//                 var detectedFormat = rawCollection[0].Format;
//                 bool isIco = detectedFormat == MagickFormat.Icon || detectedFormat == MagickFormat.Ico;
//                 bool isAnimated = detectedFormat == MagickFormat.Gif ||
//                                   detectedFormat == MagickFormat.WebP ||
//                                   detectedFormat == MagickFormat.Avif;
...
//                     foreach (var img in rawCollection) { img.Dispose(); }
//                     rawCollection.Clear();
...
//                 // PURE ZERO-LOSS PASSTHROUGH (Skips Temp file entirely, points directly to source)
//                 if (quality == 100 && input.Rotation == RotationSteps.None && rawCollection.Count == 1 && isNativePdfSupported)
//                 {
//                     processedFrames.Add(new ProcessedFrameData(input.FilePath, rawCollection[0].Width, rawCollection[0].Height));
//                     continue; 
//                 }
...
//                         switch (input.Rotation)
//                         {
//                             case RotationSteps.Right90: magickImg.Rotate(90); break;
//                             case RotationSteps.UpsideDown180: magickImg.Flip(); magickImg.Flop(); break;
//                             case RotationSteps.Left270: magickImg.Rotate(270); break;
//                         }
//                     }
...
//                     tempFileCounter++;
//                     string tempFilePath = Path.Combine(tempDirectory, $"frame_{tempFileCounter}");
...
//                             tempFilePath += ".jpg";
//                             solidCanvas.Write(tempFilePath);
//                         }
//                         else
//                         {
//                             magickImg.Format = MagickFormat.Jpeg;
...
//                         if (magickImg.HasAlpha)
//                         {
//                             // If it has transparency, we must use PNG. 
//                             // Using level '1' makes compression lightning fast. Quality remains completely lossless.
//                             magickImg.Format = MagickFormat.Png;
//                             magickImg.Settings.SetDefine(MagickFormat.Png, "compression-level", "1");
...
//                         else
//                         {
//                             // If no transparency exists, Max Quality JPEG processes 100x faster than PNG 
//                             // and natively binds to PDFs better.
//                             magickImg.Format = MagickFormat.Jpeg;
//                             magickImg.Quality = 100;
//                             magickImg.Settings.SetDefine(MagickFormat.Jpeg, "sampling-factor", "4:4:4");
...
//                     // Record the lightweight file location and dimensions to memory
//                     processedFrames.Add(new ProcessedFrameData(tempFilePath, magickImg.Width, magickImg.Height));
//                 }
                
//                 // When this iteration finishes, 'rawCollection' is disposed, and all RAM is freed!
//             }

//             // ====================================================================================
//             // PHASE 2: THE PDF ASSEMBLER
//             // ====================================================================================
//             // All heavy lifting is done. PdfSharp now just zips through the perfect files on disk.
//             using var document = new PdfDocument();
//             document.Info.Title = Path.GetFileNameWithoutExtension(finalPath);
//             document.ViewerPreferences.FitWindow = true;

//             using var runtime = new RuntimeResourceBag();

//             foreach (var frame in processedFrames)
//             {
//                 var fs = new FileStream(frame.FilePath, FileMode.Open, FileAccess.Read, FileShare.Read);
//                 runtime.Add(fs); // Will be safely closed AFTER document.Save()

//                 var xImage = XImage.FromStream(fs);
//                 xImage.Interpolate = false;
//                 runtime.Add(xImage);

//                 var page = document.AddPage();
                
//                 CalculateGeometryAndRender(page, xImage, frame.PixelWidth, frame.PixelHeight, pageSize, orientation, margin, imageFit);
//             }

//             // Lock the files into the final PDF.
//             document.Save(finalPath);
            
//             // Dispose of the RuntimeBag manually right here so that FileStreams let go of the Temp Files.
//             runtime.Dispose(); 
...
//             // PHASE 3: CLEANUP
//             // ====================================================================================
//             // Completely wipe the temporary directory from the hard drive.
//             if (Directory.Exists(tempDirectory))
//             {
//                 try { Directory.Delete(tempDirectory, true); }
//                 catch { /* Swallow cleanup exceptions (OS might occasionally lock temp files) */ }
//             }
//         }
//     }
...
//         if (imgWidth <= 0.01) imgWidth = 1;
//         if (imgHeight <= 0.01) imgHeight = 1;
